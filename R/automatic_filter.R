...
#' @export
setMethod(
    "automatic_filter", signature(so = "Seurat"),
    function(so, up_per = 0.95, low_per = 0.05, mit_per = 0.95, ...) {
        if (low_per >= up_per) {
        ...
        subset(so,
            subset = nFeature_RNA > low_per_val &
                nFeature_RNA < up_per_val &
                percent.mt < mit_val,
            ...
        )
    }
