#' @describeIn prediction_diagnostics method to plot different
#' visualization tools useful to asses the accuracy of the predictions in
#' SeuratObjects
...
setMethod("prediction_diagnostics", signature(so = "Seurat"), function(so) {
    if (!is.null(so@misc$predictions)) {
        ...
        # Heatmap of per-cell scores across reference labels - look for
        # clean diagonal blocks
        plot_prediction_scores(predictions)

        # Cells where the top label wasn't clearly better than the
        # runner-up get pruned to NA
        ...
        if (length(score_cols) == 0) {
            stop("no prediction.score columns were found in the meta.data. ",
                "Make sure sc_deconvolute was used with a SeuratObject ",
                "reference.",
                call. = FALSE
            )
        }
        ...
        heatmap(t(scores[order(so$predicted.id), , drop = FALSE]),
            Colv = NA,
            scale = "none", labCol = FALSE, main = "Prediction scores per cell"
        )

        # Cells whose winning label score didn't clear 0.5 are considered
        # low-confidence
        summary(so$prediction.score.max < 0.5)
    } else {
        stop("no prediction metadata was present in the SeuratObject. ",
...
#' @describeIn prediction_diagnostics method to plot
#' different visualization tools useful to asses the accuracy of the
#' predictions in SUmmarizedExperiments
#' @importFrom SingleR plotScoreHeatmap
#' @importFrom SummarizedExperiment colData
#' @export
setMethod(
    "prediction_diagnostics", signature(so = "SummarizedExperiment"),
    function(so) {
