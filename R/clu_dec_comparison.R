#' @describeIn clu_dec_comparison method to compare the clustering and
#' the deconvolution of \code{SeuratObjects}
#' @importFrom pheatmap pheatmap
#' @export
setMethod("clu_dec_comparison", signature(so = "Seurat"), function(so) {
    if (!"seurat_clusters" %in% colnames(so@meta.data)) {
        stop("no clusters were found in the SeuratObject. ",
            "Make sure sc_clustering was used on the object first.",
            call. = FALSE
        )
    }
    ...
    if ("predicted.id" %in% colnames(so@meta.data)) {
        ...
        stop("no predicted labels were found following standard naming ",
